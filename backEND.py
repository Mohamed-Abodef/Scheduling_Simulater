...
def output (fcfs,rr,sjf,srf,edf):
    ...
    for T in fcfs.burst_time.keys():
        fcfs_table.add_row([T,fcfs.start_t[T],fcfs.end_t[T],fcfs.burst_time[T],fcfs.state[T]])
    fcfs_table.vrules = 2

    rr_table = PrettyTable(["Task","Start Time","End Time","Duration","Status"])
    for T in rr.burst_time.keys():
        rr_table.add_row([T,rr.start_t[T], rr.end_t[T] ,rr.burst_time[T],rr.state[T]])
    rr_table.vrules = 2

    sjf_table = PrettyTable(["Task","Start Time","End Time","Duration","Status"])
    for T in sjf.burst_time.keys():
        sjf_table.add_row([T,sjf.start_t[T],sjf.end_t[T],sjf.burst_time[T],sjf.state[T]])
    sjf_table.vrules = 2

    srf_table = PrettyTable(["Task","Start Time","End Time","Duration","Status"])
    for T in srf.burst_time.keys():
        srf_table.add_row([T,srf.start_t[T],srf.end_t[T],srf.burst_time[T],srf.state[T]])
    srf_table.vrules = 2

    edf_table = PrettyTable(["Task","Start Time","End Time","Duration","Status"])
    for T in edf.burst_time.keys():
        edf_table.add_row([T,edf.start_t[T],edf.end_t[T],edf.burst_time[T],edf.state[T]])
    edf_table.vrules = 2
